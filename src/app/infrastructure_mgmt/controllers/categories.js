(function (angular, _) {
    ...
    angular.module("mfl.infrastructure_mgmt.controllers.categories", [
        "ui.router",
        "mfl.common.forms",
    ...
    .controller("mfl.infrastructure_mgmt.controllers.categories_list", ["$scope", function($scope) {
        $scope.filters = {
            ...
            $log, wrappers, forms, toasty) {
                    ...
                    if (!_.isEmpty(changed)) {
                        wrappers.categories.update($scope.category_id, changed)
                            .success(function () {
                                ...
                                $state.go(
                                    "infrastructure_mgmt.categories_list",
                ...
                $scope.remove = function () {
                    wrappers.categories.remove($scope.category_id).success(function() {
                        ...
                        $state.go("infrastructure_mgmt.category_list", {}, {reload: true});
                    }).error(function(data) {
                        $scope.errors = data;
                    });
                };
                $scope.cancel = function () {
                    $state.go("infrastructure_mgmt.categories_list.category_edit");
                }
            }])

        .controller("mfl.infrastructure_mgmt.controllers.categories_create", 
            ["$scope", "$state", "$log", "mfl.infrastructure_mgmt.wrappers", "toasty",
