(function(angular, _) {
            ...
            $scope.remove_contact = function(obj) {
                if (!_.isUndefined(obj.id)) {
                    ...
                    $scope.facilityRegulatoryBodies.contacts =
                        _.without($scope.facilityRegulatoryBodies.contacts, obj);
                }
            };
        }
    ])
    ...
    .controller("mfl.setup.controller.facility_types.list", ["$scope", function($scope) {
        $scope.filters = { "fields": "id,name,sub_division", is_parent: true };
    }])
    ...
    .controller("mfl.setup.controller.regulation_statuses.view", ["$scope", "adminApi",
        ...
        .controller("mfl.setup.controller.facility_types_details.list", ["$scope",
            function($scope) {
                $scope.filters = { "fields": "id,name,sub_division", is_parent: false };
            }
        ])
        .controller("mfl.setup.controller.facility_types_details.create", ["$scope", "adminApi",
            "$stateParams", "$state", "toasty",
            function($scope, adminApi, $stateParams, $state, toasty) {
                adminApi.facility_types.filter({ is_parent: true })
                    .success(function(data) {
                        $scope.facility_types_parents = data.results;
                    })
                    .error(function(err) {
                        $scope.errors = err;
                    });
                $scope.facility_type_details = {
                    name: " ",
                    sub_division: " "
                };
                if ($stateParams.type_id) {
                    adminApi.facility_types.get($stateParams.type_id)
                        .success(function(data) {
                            $scope.facility_type_details = data;
                        })
                        .error(function(err) {
                            $scope.errors = err;
                        });
                ...
                $scope.saveFacilityType = function() {
                    if (!$stateParams.type_id){
                        adminApi.facility_types.create($scope.facility_type_details)
                        .success(function() {
                            toasty.success({
                                title: "Facility Type Detail",
                                msg: "Facility type detail has been created"
                            });
                            $state.go("setup.facility_types_details");
                        })
                        .error(function(err) {
                            $scope.errors = err;
                        });
                    ...
                    if ($stateParams.type_id){
                        adminApi.facility_types.update(
                            $stateParams.type_id, $scope.facility_type_details)
                        .success(function() {
                            toasty.success({
                                title: "Facility Type Detail",
                                msg: "Facility type detail has been Updated"
                            });
                            $state.go("setup.facility_types_details");
                        })
                        .error(function(err) {
